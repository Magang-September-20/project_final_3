...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 22, 2020 at 08:56 AM
...
CREATE TABLE `tb_m_education` (
  `id` int(4) NOT NULL,
  `degree` enum('D3','S1','S2','S3','') NOT NULL DEFAULT '',
  `status` enum('Lulus','Belum Lulus','') NOT NULL DEFAULT '',
  `ipk` float(3,2) NOT NULL DEFAULT 0.00,
  `major` int(4) NOT NULL DEFAULT 11,
  `university` int(4) NOT NULL DEFAULT 18
...
CREATE TABLE `tb_m_file` (
  `id` int(4) NOT NULL,
  `cv` varchar(225) NOT NULL DEFAULT '',
  `photo` varchar(225) NOT NULL DEFAULT ''
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
(17, 'liv', 'recruitmentnapao3@gmail.com', 'Perempuan', '2020-10-01');

--
-- Triggers `tb_m_user`
--
DELIMITER $$
CREATE TRIGGER `tg_after_insert_user` AFTER INSERT ON `tb_m_user` FOR EACH ROW BEGIN
INSERT INTO tb_m_education SET id = new.id;
INSERT INTO tb_m_file SET id = new.id;
END
$$
DELIMITER ;
